...
Credits: written by AYUSH JHA {@ayushjha_2911}
"""
import asyncio
import io 
...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "No name set yet nibba, check pinned message in @ayushtechy"
PREV_REPLY_MESSAGE = {}
        ...
        async with borg.conversation(chat) as conv:
         ...
         if y == "1":
             ...
             if x == "1":
                 ...
                 await borg.send_message(chat, "**Whenever I see Your Message. I will reply you when i get online.**")
                 response = await conv.get_response(chat)
                 if not response.text == "/start":
                     await borg.send_message(chat, LWARN)
                     response = await conv.get_response(chat)
                 ...
                 await borg.send_message(chat, "`Please Stop Now`")
                 response = await conv.get_response(chat)
                 if not response.text == "/start":
                     await borg.send_message(chat, LWARN)
                     response = await conv.get_response(chat)
